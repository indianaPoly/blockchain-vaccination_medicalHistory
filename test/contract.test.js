...
describe("Contract Test - Parent-Child Relationship", () => {
  ...
  beforeEach(async () => {
    ...
    MedicalHistory = await hre.ethers.getContractFactory(
      "MedicalHistory"
    );
    medicalHistory = await MedicalHistory.deploy();
    console.log("진료 내역 컨트랙트가 정상적으로 배포되었습니다.");
    console.log("진료 내역 컨트랙트 주소: ", await medicalHistory.getAddress(), "\n");
    ...
    console.log("예뱡 접종 컨트랙트 주소: ", await vaccinationManagement.getAddress(), "\n");
  ...
  it("should add and link children", async () => {
    // owner가 두 명의 자녀를 추가
    await parentChildRelationship.connect(owner).createChild(
      "Go Hyunlim",
      20231030,
      10.1 * 10,
      90.1 * 10
    ...
    const secondChildAddress = await parentChildRelationship.connect(owner).returnChildAddress("Go Hwirim");
    ...
    let child1_healthData_parent1 = await parentChildRelationship.connect(owner).getHealthInformation(firstChildAddress);
    const child2_healthData_parent1 = await parentChildRelationship.connect(owner).getHealthInformation(secondChildAddress);
    ...
    let child1_healthData_parent2 = await parentChildRelationship.connect(secondaryAddress).getHealthInformation(firstChildAddress);
    ...
    await parentChildRelationship.connect(owner).setHealthInformation(firstChildAddress, 175.5*10, 80.7*10);
    console.log("자녀1의 키와 몸무게 수정을 하였습니다.", "\n");

    console.log("수정한 이후의 데이터를 부모1, 부모 2 둘 다 조회");
    child1_healthData_parent1 = await parentChildRelationship.connect(owner).getHealthInformation(firstChildAddress);
    child1_healthData_parent2 = await parentChildRelationship.connect(secondaryAddress).getHealthInformation(firstChildAddress);
    ...
    await parentChildRelationship.connect(owner).addMedicalHistoryForChild(
      firstChildAddress,
      0,
      "세브란스 병원",
      "2024.10.10",
    ...
    let child1_medicalHistory_parent1 = await parentChildRelationship.connect(owner).getMedicalHistoriesForChild(firstChildAddress);
    console.log(child1_medicalHistory_parent1, "\n");

    console.log("부모 2가 자녀 1에 대한 진료 내역 조회");
    let child1_medicalHistory_parent2 = await parentChildRelationship.connect(secondaryAddress).getMedicalHistoriesForChild(firstChildAddress);
    console.log(child1_medicalHistory_parent2);

    await parentChildRelationship.connect(secondaryAddress).addMedicalHistoryForChild(
      firstChildAddress,
      0,
      "차병원",
      "2024.11.10",
    ...
    console.log("부모 1이 자녀 1에 대한 진료 내역 조회");
    child1_medicalHistory_parent1 = await parentChildRelationship.connect(owner).getMedicalHistoriesForChild(firstChildAddress);
    console.log(child1_medicalHistory_parent1, "\n");

    console.log("부모 2가 자녀 1에 대한 진료 내역 조회");
    child1_medicalHistory_parent2 = await parentChildRelationship.connect(secondaryAddress).getMedicalHistoriesForChild(firstChildAddress);
    console.log(child1_medicalHistory_parent2);
  });
});
